...
o.numFMRs = 0;                    //total number of FMR files
o.numBOLDFMRs = 0;                //total number of actual functional runs
...
o.volRefFileName = "/home/tarrlab/BrainVoyager-Analysis-Tools/BV-Scripts/STS-R21-Volume-Reference.txt";
...
o.vdmEstimate = ["*OppPhase*PA*.fmr", ""];					//filename identifier for opposite phase run for VDM estimation
...
o.use_ic = 0;		//type of data (if 0, gradient echo)
...
o.save_derivative = 0;  //save derivative
...
o.createFMRs = function() {
    ...
    BrainVoyager.PrintToLog("Found " + this.numFMRs + " BOLD runs");
...
o.runBBR = function() {
    ...
    //look for merged white-matter srf and vwp files, rename them
    var checkStr = "_Merged_smoothwm";
    var bvd = new QDir(this.BVdir);
    var srfList = bvd.entryList(["*" + checkStr + ".srf"]);
    var vwpList = bvd.entryList(["*" + checkStr + ".vwp"]);
    if(srfList.length == 0){
      BrainVoyager.PrintToLog("Error: no .srf file found. Aborting BBR.");
      return;
    }
    else if(vwpList.length == 0){
      BrainVoyager.PrintToLog("Error: no .vwp file found. Aborting BBR.");
      return;
    }
    var srfName = srfList[0];
    var vwpName = vwpList[0];
    BrainVoyager.PrintToLog("Found .srf and .vwp files: " + srfName + " and " + vwpName);
    var srfInfo = new QFileInfo(this.BVdir + "/" + srfName);
    var vwpInfo = new QFileInfo(this.BVdir + "/" + vwpName);
    var newSrfName = srfName.substring(0, srfName.indexOf(checkStr)) + "-Anat_ETC-7x-R5_WM_RECOSM_D300k.srf";
    var newVwpName = vwpName.substring(0, vwpName.indexOf(checkStr)) + "-Anat_ETC-7x-R5_WM_RECOSM_D300k.vwp";
    BrainVoyager.PrintToLog("Renamed .srf and .vwp files: " + newSrfName + " and " + newVwpName);
    bvd.rename(srfInfo.absoluteFilePath(), newSrfName);
    bvd.rename(vwpInfo.absoluteFilePath(), newVwpName);
...
o.createSessionListFile = function() {
  ...
  ts.writeString(this.faFile + "\n");
  ts.writeString(this.iaFile + "\n");
...
o.createVTCs = function() {
  ...
  BrainVoyager.PrintToLog("Got VMR: " + VMR_path);
  var FA_path = textstr.readLine();
  BrainVoyager.PrintToLog("Got FA: " + FA_path);
  var IA_path = textstr.readLine();
  BrainVoyager.PrintToLog("Got IA: " + IA_path);
  var fmr_regex = /.fmr/;
  var name_regex = /.*[^(.fmr)]/;
  ...
  for (filecounter = 0; filecounter < nroffiles; filecounter++) {
    ...
    BrainVoyager.PrintToLog("Got line: " + filename);
  ...
  for (i=0; i<new_number; i++) {
    ...
    //VTC Params: 2,2,2,100 : 2 = float, 2 = resolution (2mm), 2 = sinc interpolation, 100 = threshold if no .bbx
...
o.findVolRef = function(runName){
    for(var count = 0; count < this.taskIDs.length; count++){
      BrainVoyager.PrintToLog("DEBUG: current taskID comparison: " + this.taskIDs[count]);
    ...
    if(this.vdmEstimate[1].length == 0){
        if(this.mCorrTarget.length > 0){
          ...
          var mcFile = new QFile(this.BVdir + "/" + this.mCorrTargetFileName);
        	...
        	var is = new QTextStream(mcFile);
        	var mcf = is.readLine();
          BrainVoyager.PrintToLog("~~~~~DEBUG: o.estimate: line read from motion correction target file log: " + mcf);
        	if(mcf){
        		this.mCorrTarget = mcf;
...
//this check is only needed for spin echo VDM map estimation;
//left out of current (gradient echo) version
/*
...
*/
    ...
    BrainVoyager.PrintToLog("Found PA Opposite Phase and AP functional target runs: " + file0 + " and " + file1);
...
o.vdmBrowse = function() {
  ...
  if(this.vdmEstimate[1].length == 0){
  	...
  	var mcf = is.readLine();
  	if(mcf){
      BrainVoyager.PrintToLog("~~~~~DEBUG: o.vdmBrowse: line read from motion corr target file: " + mcf);
  		this.vdmEstimate[1] = this.getFileFromPath(mcf);
      BrainVoyager.PrintToLog("~~~~~DEBUG: vdmBrowse: set vdm target identifier as " + this.vdmEstimate[1]);
  	}
  	else{
  		BrainVoyager.PrintToLog("ERROR: no motion correction target filename specified");
  	}
  ...
  //AP file identifier should be vdmEstimate[1] - we want top apply the AP VDM map
  //to all functional runs, as they were collected with AP phase encoding
  var baseName = this.vdmEstimate[1];
  BrainVoyager.PrintToLog("~~~~~DEBUG: vdmBrowse: vdm base name is " + baseName);
  var fname = baseName.substring(0, baseName.length-4);
  var fcheck = fname + "_vdm.map";
  BrainVoyager.PrintToLog("~~~~~DEBUG: vdmBrowse: looking for vdm map named " + fcheck);
...
o.apply = function() {
    ...
    //new method to avoid crash - call COPE for each application, not batch
    for(var f = 0; f < files.length; f++){

      bvqx.PrintToLog("Calling COPE -- applying VDM to " + files[f]);

      bvqx.SetPluginIntParameter("rllr", this.rllr);
      bvqx.SetPluginStringParameter("vdm", this.vdmFile);
      bvqx.SetPluginStringParameter("files", files[f]);
